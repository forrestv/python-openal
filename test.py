...
buffer = openal.Buffer('440.wav')
buffer2 = openal.Buffer('water.wav')
expbuffers = [openal.Buffer(os.path.join('explodes', x)) for x in os.listdir('explodes')]
...
while True:
    ...
    source.position = amplitude*math.sin(t*freq), amplitude*math.cos(t*freq), amplitude2*math.sin(t*freq2)
